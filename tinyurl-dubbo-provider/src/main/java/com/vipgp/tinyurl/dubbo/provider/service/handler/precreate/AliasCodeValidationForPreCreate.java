...
public class AliasCodeValidationForPreCreate extends AbstractCreateValidation implements Checker<CreateDTO> {
    ...
    public BaseResult doProcess(String baseUrl, String rawUrl,String aliasCode) {
        // alias code
        BaseResult result = validator.validateCode(aliasCode);
        return result;
    }

    @Override
    public void check(CreateDTO createDTO) {
        BaseResult result=validator.validateCode(createDTO.getAliasCode());
        if(result!=null){
